<template>
	...
	<div class="forum-detail" v-if="content">
		...
		<!-- <div class="comment-submit">发表评论：</div>
			...
			<h5 class="give_score">打分：</h5>
			<el-rate style = "padding: 15px 10px;"> v-model="score" :max="5" show-text allow-half @change="handleScoreChange"></el-rate>
			<p style = "padding: 15px 10px;">您的评分是: {{ score }}</p>
			<el-button type="primary" @click="submitComment" class="submit-button">发 表</el-button>
		</div>
	
		<el-dialog :visible.sync="dialogVisible" v-model="dialogVisible" title=" 评 论 " width="45%" center :close-on-click-modal="false">
			<el-input v-model="sendNewComment" type="textarea" :rows="3" placeholder="请输入评论内容" class="comment-input"></el-input>
	
			 <div slot="footer" class="dialog-footer">
			 	<el-button @click="dialogVisible = false">取消</el-button>
			 	<el-button type="primary" @click="sendComment" class = "submit_buttom"> 发 表 </el-button>
			 </div>
		</el-dialog>
	</template> -->
	...
	methods: {
		clickLike(row) {
			if (this.userid == '' || this.userid == null) {
				...
				axios.post('/api/clickLike', { 'comment_id': row.id, 'section': 'info', 'user_id': this.userid }, { headers: { "Access-Control-Allow-Origin": "*" } })
					.then(response => {
		...
		clickLikeCancel(row) {
			if (this.userid == '' || this.userid == null) {
				...
				axios.post('/api/clickLikeCancel', { 'comment_id': row.id, 'section': 'info', 'user_id': this.userid }, { headers: { "Access-Control-Allow-Origin": "*" } })
					.then(response => {
						//console.log('2222=',this.currentPost);
						ElNotification({
							title: '系统提示',
		...
		sendComment() {
			if (this.userid == '' || this.userid == null) {
				...
				if (this.sendNewComment == '') {
					...
					//此处paperid为当前评论ID
					axios.post('/api/add_comment', { 'paper_id': this.commentId, 'user_id': this.userid, 'section': 'info_new', 'newComment': this.sendNewComment }, { headers: { "Access-Control-Allow-Origin": "*" } })
						.then(response => {
							console.log('2222=', this.currentPost);
							// ElNotification({
							//   title: '系统提示',
		...
		submitComment() {
			...
			if (this.userid == '' || this.userid == null) {
				...
				if (this.newComment == '') {
					...
					axios.post('/api/add_comment', { 'paper_id': this.paperid, 'user_id': this.userid, 'section': 'info', 'newComment': this.newComment, 'score': this.score }, { headers: { "Access-Control-Allow-Origin": "*" } })
						.then(response => {
							console.log('2222=', this.currentPost);
							// ElNotification({
							//   title: '系统提示',
		...
		handlePostTitleClick(post) {
			...
			axios.post('/api/paper_content', { 'paper_id': post.id, 'section': 'info', 'user_id': this.userid }, { headers: { "Access-Control-Allow-Origin": "*" } })
				.then(response => {
		...
		fetchData() {
			axios.post('/api/paper_list', { 'section': 'info', 'keyword': this.keyword }, { headers: { "Access-Control-Allow-Origin": "*" } })
				.then(response => {
					//console.log(response);
					if (response.data.code == 0) {
